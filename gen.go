...
func main() {
	...
	// Execute the template in memory so invalid or unformatted output never
	// replaces the last valid generated file.
	var generated bytes.Buffer
	err = parsedTemplate.ExecuteTemplate(&generated, "model.tmpl", templateData{
	...
	if err != nil {
		log.Fatalf("format generated source: %s", err)
	}
	if err := os.WriteFile(path.Join(wd, "azurecaf/models_generated.go"), formatted, 0644); err != nil {
		log.Fatal(err)
	}
