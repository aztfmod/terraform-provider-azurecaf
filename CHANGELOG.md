...
### Fixed
- **Agentic workflow model routing**: Pinned all Copilot workflows to the supported `gpt-5.6-luna` model. After organization-billed authentication was enabled, the `auto` alias resolved to the internal `gpt-5.6-luna-utility` model, which the Copilot chat-completions endpoint rejected with HTTP 400 and caused PR Review Agent and Contributor Welcome CI failures. Impact: Low — workflow model selection only; provider behavior is unchanged.
...
- **Revamped test-suite CI targets and workflow tiers**: Added Makefile targets for the new fast unit suite (`test_unit_fast`), golden snapshots (`test_golden`, `test_golden_update`), fuzzing (`test_fuzz`), and regression baselines (`test_regression`, `test_regression_update`). Split `.github/workflows/go.yml` into build, fast-tier unit, medium-tier integration, and PR-only E2E jobs, and expanded `.github/workflows/nightly-regression.md` to run golden, fuzz, and regression checks nightly.
  - Impact: Low — CI/test tooling only, no provider runtime behavior change.
...
### Security
- **Agentic workflow authentication**: Migrated all Copilot-powered GitHub Agentic Workflows from the `COPILOT_GITHUB_TOKEN` personal access token flow to GitHub's built-in, per-run authentication by granting `copilot-requests: write`. With centralized Copilot CLI billing enabled in the organization, GitHub now mints and revokes an ephemeral token for each workflow run, eliminating the long-lived inference PAT and repository-secret management requirement. Regenerated the committed workflow lock files with `gh aw` v0.88.7 and explicitly disabled unused shell access in the two untrusted-content workflows required by current strict-mode validation. Impact: Low — authentication and workflow-hardening changes only; provider runtime behavior is unchanged.
